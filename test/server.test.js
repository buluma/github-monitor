...
test("conflicted pull requests are excluded from CI status buckets", () => {
  ...
  assert.deepEqual(
    groups.pass.map((pr) => pr.number),
    [1],
  );
  assert.deepEqual(
    groups.fail.map((pr) => pr.number),
    [],
  );
  assert.deepEqual(
    groups.running.map((pr) => pr.number),
    [],
  );
  assert.deepEqual(
    groups.conflicts.map((pr) => pr.number),
    [2, 3, 4],
  );
});
...
test("running Actions evidence moves PRs out of the no-CI bucket", () => {
  ...
  assert.deepEqual(
    groups.running.map((row) => row.number),
    [297],
  );
});
...
test("open pull request searches exclude archived repositories", () => {
  assert.equal(
    openPullRequestSearchQuery("owner", "dev"),
    "is:pr state:open archived:false owner:dev",
  );
  assert.equal(
    openPullRequestSearchQuery("author", "dev"),
    "is:pr state:open archived:false author:dev",
  );
});
...
test("Dependabot workflow detection is exact and queue depth deduplicates runs", () => {
  ...
  assert.equal(
    isDependabotWorkflowRun({ actor: { login: "dependabot[bot]" } }),
    true,
  );
  assert.equal(
    isDependabotWorkflowRun({ triggering_actor: { login: "dependabot[bot]" } }),
    true,
  );
  assert.equal(
    isDependabotWorkflowRun({ actor: { login: "maintainer" } }),
    false,
  );
  assert.equal(
    dependabotQueueDepth([
...
test("failed Dependabot runs arrive pre-dismissed only when cleanup is enabled", () => {
  ...
  assert.deepEqual(
    markAutoDismissedDependabotRuns(undefined, { enabled: true }),
    [],
  );
});
...
test("refresh recommendations pause when GitHub API quota is low", () => {
  ...
  assert.ok(
    new Date(refresh.nextRefreshAt).getTime() >= new Date(resetAt).getTime(),
  );
});
  ...
  const best = bestProductionUrlCandidate(
    [
      ...
      },
      {
        ...
        source: "infra/cdk/app-stack.ts",
      },
      {
        url: "safespendplan-market-refresh.json",
      ...
      {
        url: "certificate.domainvalidationoptions",
        source: "infra/cdk/app-stack.ts",
      },
...
test("productionTargetFromCodeFiles honors source precedence", () => {
  ...
  const fromConfig = productionTargetFromCodeFiles({
    "next.config.js":
      "module.exports = { env: { SITE: 'https://prod.acme-app.io' } }",
  });
...
test("finished CD change summaries infer reviewable page links and cues", () => {
  assert.equal(
    publicRouteFromFile("app/settings/billing/page.tsx"),
    "/settings/billing",
  );
      ...
      files: [
        {
          ...
          changes: 12,
          blob_url:
            "https://github.com/acme/app/blob/abcdef1/app/settings/billing/page.tsx",
        },
        {
          ...
          changes: 10,
          blob_url:
            "https://github.com/acme/app/blob/abcdef1/app/settings/billing/styles.css",
        },
      ],
    },
  ...
  assert.equal(
    summary.mergedPullRequests[0].changedPages[0].url,
    "https://app.example.com/settings/billing",
  );
  ...
  assert.equal(
    commitFallback.recentCommits[0].changedPages[0].url,
    "https://app.example.com/",
  );
  assert.equal(
    commitFallback.reviewLinks.commitsUrl,
    "https://github.com/acme/app/commits/main",
  );
          ...
          files: [
            {
              ...
              deletions: 1,
              blob_url:
                "https://github.com/acme/app/blob/head/pages/dashboard.tsx",
            },
          ],
        },
      ],
  ...
  assert.equal(
    homepageTarget.mergedPullRequests[0].changedPages[0].url,
    "https://prod.example.com/dashboard",
  );
      ...
      mergedPullRequests: [
        {
          pr: {
            number: 118,
            title:
              "security: fix forgeable JWT key, magic-link race, plan IDOR",
            user: { login: "cigan1" },
            merged_at: "2026-05-20T12:00:00Z",
          ...
          files: [
            {
              ...
              deletions: 4,
              blob_url:
                "https://github.com/GipsyChef/safespendplan/blob/head/src/auth/magic-link.ts",
            },
          ],
        },
      ],
  ...
  assert.equal(
    inferredTarget.mergedPullRequests[0].changedPages[0].url,
    "https://safespendplan.com/login",
  );
...
test("runner status endpoint returns only busy runners", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    const headers = {
      ...
      "x-ratelimit-resource":
        requestUrl.pathname === "/graphql" ? "graphql" : "core",
    };
  ...
  try {
    ...
    const response = await previousFetch(
      `http://127.0.0.1:${port}/api/runners/status?mode=all&jobs=1`,
    );
...
test("dashboard includes running non-CD workflow runs in CI running work", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/repos/acme/app/actions/runs/45/jobs") {
      return Response.json(
        {
          jobs: [{ name: "engine — pytest", conclusion: "failure" }],
        },
        { headers },
      );
    }
...
test("deep queue cleanup closes Dependabot PRs and cancels every active Dependabot run", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (
      requestUrl.pathname === "/repos/deep-queue-fixture/app/pulls" &&
      options.method === "GET"
    ) {
      return Response.json(
        [
          {
            number: 7,
            title: "Bump runtime",
            html_url: "https://github.com/deep-queue-fixture/app/pull/7",
          ...
          },
          {
            number: 8,
            title: "Human change",
            html_url: "https://github.com/deep-queue-fixture/app/pull/8",
    ...
    if (
      requestUrl.pathname ===
      "/repos/deep-queue-fixture/app/commits/sha7/check-runs"
    ) {
      return Response.json(
    ...
    if (
      requestUrl.pathname ===
      "/repos/deep-queue-fixture/app/commits/sha7/status"
    ) {
      return Response.json({ statuses: [] }, { headers });
    }
    if (
      requestUrl.pathname === "/repos/deep-queue-fixture/app/actions/runs" &&
      options.method === "GET"
    ) {
          ...
          ? [
              {
                ...
                status,
                html_url:
                  "https://github.com/deep-queue-fixture/app/actions/runs/70",
              ...
              },
              {
                ...
                status,
                html_url:
                  "https://github.com/deep-queue-fixture/app/actions/runs/71",
            ...
            ? [
                {
                  ...
                  status,
                  html_url:
                    "https://github.com/deep-queue-fixture/app/actions/runs/72",
    ...
    if (
      requestUrl.pathname === "/repos/deep-queue-fixture/app/pulls/7" &&
    ...
    if (
      /\/repos\/deep-queue-fixture\/app\/actions\/runs\/(70|72)\/cancel/.test(
        requestUrl.pathname,
      ) &&
      options.method === "POST"
    ) {
  ...
  try {
    const result = await cleanupDependabotWorkload({
      repos: ["deep-queue-fixture/app"],
      jobs: 2,
    });
    assert.deepEqual(result.errors, []);
    assert.deepEqual(
      result.closedPullRequests.map((pr) => pr.number),
      [7],
    );
    assert.deepEqual(
      result.cancelledRuns.map((run) => run.runId).sort(),
      [70, 72],
    );
...
test("cleanup preserves successful discoveries when one active-status request fails", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (
      requestUrl.pathname === "/repos/partial-cleanup-fixture/app/pulls" &&
      options.method === "GET"
    ) {
      return Response.json(
        [
          {
            ...
            html_url: "https://github.com/partial-cleanup-fixture/app/pull/9",
            user: { login: "dependabot[bot]" },
            head: { sha: "sha9" },
          },
        ],
    ...
    if (
      requestUrl.pathname ===
      "/repos/partial-cleanup-fixture/app/commits/sha9/check-runs"
    ) {
      return Response.json(
        { check_runs: [{ status: "completed", conclusion: "failure" }] },
        { headers },
    ...
    if (
      requestUrl.pathname ===
      "/repos/partial-cleanup-fixture/app/commits/sha9/status"
    ) {
      return Response.json({ statuses: [] }, { headers });
    }
    if (
      requestUrl.pathname ===
        "/repos/partial-cleanup-fixture/app/actions/runs" &&
      options.method === "GET"
    ) {
      const status = requestUrl.searchParams.get("status");
      if (status === "waiting")
    ...
    if (
      requestUrl.pathname === "/repos/partial-cleanup-fixture/app/pulls/9" &&
      options.method === "PATCH"
    ) {
      mutations.push(`${options.method} ${requestUrl.pathname}`);
      return Response.json({ state: "closed" }, { headers });
    ...
    if (
      requestUrl.pathname ===
        "/repos/partial-cleanup-fixture/app/actions/runs/90/cancel" &&
      options.method === "POST"
    ) {
      mutations.push(`${options.method} ${requestUrl.pathname}`);
      return new Response(null, { status: 202, headers });
  ...
  try {
    const result = await cleanupDependabotWorkload({
      repos: ["partial-cleanup-fixture/app"],
      jobs: 2,
    });
...
test("cleanup stops before mutations when discovery exhausts GitHub quota", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    const remaining = requestUrl.pathname.endsWith("/actions/runs")
      ? "0"
      : "1000";
    const headers = {
      "content-type": "application/json",
    ...
    if (
      /^\/repos\/quota-cleanup-fixture\/app-\d+\/pulls$/.test(
        requestUrl.pathname,
      ) &&
      options.method === "GET"
    ) {
    ...
    if (
      /^\/repos\/quota-cleanup-fixture\/app-\d+\/actions\/runs$/.test(
        requestUrl.pathname,
      ) &&
      options.method === "GET"
    ) {
  ...
  try {
    const repos = Array.from(
      { length: 10 },
      (_, index) => `quota-cleanup-fixture/app-${index + 1}`,
    );
    ...
    assert.equal(
      result.errors.some((error) => /quota/i.test(error)),
      true,
    );
    assert.deepEqual(mutations, []);
    assert.ok(
      getRequests <= 6,
      `expected quota abort to bound discovery requests, received ${getRequests}`,
    );
  } finally {
    resetObservedRateBuckets();
    globalThis.fetch = previousFetch;
...
test("background queue scan paginates queued runs and observes its cooldown", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/user")
      return Response.json({ login: "queue-scheduler-user" }, { headers });
    if (requestUrl.pathname === "/user/orgs")
      return Response.json([{ login: "queue-scheduler-owner" }], { headers });
    if (requestUrl.pathname === "/orgs/queue-scheduler-owner/repos") {
      return Response.json(
        [{ full_name: "queue-scheduler-owner/app", archived: false }],
        { headers },
      );
    }
    if (
      requestUrl.pathname === "/repos/queue-scheduler-owner/app/pulls" &&
      options.method === "GET"
    ) {
      return Response.json(
        [
          {
            number: 11,
            title: "Bump dependency",
            html_url: "https://github.com/queue-scheduler-owner/app/pull/11",
            user: { login: "dependabot[bot]" },
            head: { sha: "sha11" },
          },
        ],
    ...
    if (
      requestUrl.pathname ===
      "/repos/queue-scheduler-owner/app/commits/sha11/check-runs"
    ) {
      return Response.json(
        { check_runs: [{ status: "completed", conclusion: "failure" }] },
        { headers },
    ...
    if (
      requestUrl.pathname ===
      "/repos/queue-scheduler-owner/app/commits/sha11/status"
    ) {
      return Response.json({ statuses: [] }, { headers });
    }
    if (
      requestUrl.pathname === "/repos/queue-scheduler-owner/app/actions/runs" &&
      options.method === "GET"
    ) {
      const status = requestUrl.searchParams.get("status");
    ...
    if (
      requestUrl.pathname ===
        "/repos/queue-scheduler-owner/app/actions/runs/101/cancel" &&
      options.method === "POST"
    ) {
      mutations.push(`${options.method} ${requestUrl.pathname}`);
      return new Response(null, { status: 202, headers });
    ...
    if (
      requestUrl.pathname === "/repos/queue-scheduler-owner/app/pulls/11" &&
      options.method === "PATCH"
    ) {
      mutations.push(`${options.method} ${requestUrl.pathname}`);
      return Response.json({ state: "closed" }, { headers });
  ...
  try {
    ...
    assert.equal(result.closedPullRequests.length, 1);
    assert.deepEqual(
      result.cancelledRuns.map((run) => run.runId),
      [101],
    );
    assert.deepEqual(mutations.sort(), [
...
test("a shallow queue still closes Dependabot PRs with failing CI without cancelling runs", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/user")
      return Response.json({ login: "shallow-queue-user" }, { headers });
    if (requestUrl.pathname === "/user/orgs")
      return Response.json([{ login: "shallow-queue-owner" }], { headers });
    if (requestUrl.pathname === "/orgs/shallow-queue-owner/repos") {
      return Response.json(
        [{ full_name: "shallow-queue-owner/app", archived: false }],
        { headers },
      );
    }
    if (
      requestUrl.pathname === "/repos/shallow-queue-owner/app/pulls" &&
      options.method === "GET"
    ) {
      return Response.json(
        [
          {
            number: 12,
            title: "Bump failing dep",
            user: { login: "dependabot[bot]" },
            head: { sha: "fail-sha" },
          },
          {
            number: 13,
            title: "Bump green dep",
            user: { login: "dependabot[bot]" },
            head: { sha: "pass-sha" },
          },
        ],
    ...
    if (
      requestUrl.pathname ===
      "/repos/shallow-queue-owner/app/commits/fail-sha/check-runs"
    ) {
      return Response.json(
        { check_runs: [{ status: "completed", conclusion: "failure" }] },
        { headers },
    ...
    if (
      requestUrl.pathname ===
      "/repos/shallow-queue-owner/app/commits/pass-sha/check-runs"
    ) {
      return Response.json(
    ...
    if (
      /\/repos\/shallow-queue-owner\/app\/commits\/(fail|pass)-sha\/status$/.test(
        requestUrl.pathname,
      )
    ) {
      return Response.json({ statuses: [] }, { headers });
    ...
    if (
      requestUrl.pathname === "/repos/shallow-queue-owner/app/actions/runs" &&
      options.method === "GET"
    ) {
  ...
  try {
    ...
    assert.deepEqual(result.cancelledRuns, []);
    assert.deepEqual(mutations, [
      "PATCH /repos/shallow-queue-owner/app/pulls/12",
    ]);
  } finally {
    resetDependabotCleanupState();
    resetObservedRateBuckets();
...
test("failing CI detection accepts REST lowercase check runs and legacy commit statuses", () => {
  assert.equal(
    hasFailedCiSignal([{ status: "completed", conclusion: "failure" }], []),
    true,
  );
  assert.equal(
    hasFailedCiSignal(
  ...
  assert.equal(
    hasFailedCiSignal([{ status: "completed", conclusion: "skipped" }], []),
    false,
  );
  // An in-flight run that has not concluded must not read as a failure.
  assert.equal(
    hasFailedCiSignal([{ status: "in_progress", conclusion: null }], []),
    false,
  );
...
test("only the latest completed run per lane surfaces as a failure", () => {
  ...
  assert.deepEqual(
    selectFailedActionRuns(sameLaneFailures, { now }).map((r) => r.id),
    [30],
  );
  ...
  assert.deepEqual(
    selectFailedActionRuns(retriedAndDelivered, { now }).map((r) => r.id),
    [],
  );
  ...
  assert.deepEqual(
    selectFailedActionRuns(distinctLanes, { now })
      .map((r) => r.id)
      .sort(),
    [40, 41],
  );
...
test("pipeline traces flag running workflows only after four hours", () => {
  ...
  assert.ok(
    overFourHours.flagged.every((trace) => /still running/i.test(trace.reason)),
  );
});
...
test("dashboard scoreboard surfaces skipped CD runs without a new lane", () => {
  ...
  assert.match(
    indexHtml,
    /class="metric metric-green"[\s\S]*?id="metricFinishedCdSub"/,
  );
});
...
test("findSupersedingSuccessfulRun marks a failure as resolved only when a newer success exists", () => {
  ...
  const succ414 = {
    ...
    run_number: 414,
    conclusion: "success",
    html_url: "https://example/414",
  };
  const succ413 = {
    id: 413,
    run_number: 413,
    conclusion: "neutral",
    html_url: "https://example/413",
  };
  const skip413 = {
    id: 4131,
    run_number: 413,
    conclusion: "skipped",
    html_url: "https://example/413s",
  };
  const fail412 = {
    id: 412,
    run_number: 412,
    conclusion: "failure",
    html_url: "https://example/412",
  };
  const fail411 = {
    id: 411,
    run_number: 411,
...
test("conditional cache attaches If-None-Match only for GET/HEAD with a known ETag", () => {
  ...
  assert.deepEqual(
    applyConditionalHeaders(base, store, "https://api/x", "GET"),
    base,
  );
  ...
  assert.equal(
    "if-none-match" in
      applyConditionalHeaders(base, store, "https://api/x", "POST"),
    false,
  );
  assert.equal(
    "if-none-match" in
      applyConditionalHeaders(base, store, "https://api/x", "PATCH"),
    false,
  );
  assert.equal(
    "if-none-match" in
      applyConditionalHeaders(base, store, "https://api/x", "DELETE"),
    false,
  );
...
test("304 returns the cached body and any other status returns null", () => {
  ...
  assert.deepEqual(
    takeCachedConditionalResponse(store, "https://api/x", "GET", 304),
    { value: 7 },
  );
  // Different URL or method → no cached body.
  assert.equal(
    takeCachedConditionalResponse(store, "https://api/y", "GET", 304),
    null,
  );
  assert.equal(
    takeCachedConditionalResponse(store, "https://api/x", "POST", 304),
    null,
  );
  // Non-304 statuses don't read the cache.
  assert.equal(
    takeCachedConditionalResponse(store, "https://api/x", "GET", 200),
    null,
  );
  assert.equal(
    takeCachedConditionalResponse(store, "https://api/x", "GET", 404),
    null,
  );
});
...
test("storeConditionalResponse caches GET bodies with their ETag and evicts entries that no longer carry one", () => {
  ...
  assert.equal(
    storeConditionalResponse(store, "https://api/x", "GET", responseWithEtag, {
      value: 1,
    }),
    true,
  );
  assert.deepEqual(store.get("https://api/x"), {
    etag: 'W/"v1"',
    body: { value: 1 },
  });
  ...
  assert.equal(
    storeConditionalResponse(store, "https://api/x", "POST", responseWithEtag, {
      value: 3,
    }),
    false,
  );
...
test("extractOwnerFromPath returns the owner for /repos, /orgs, /users paths and null elsewhere", () => {
  assert.equal(
    extractOwnerFromPath("/repos/GipsyChef/agentdraft/pulls"),
    "GipsyChef",
  );
  assert.equal(
    extractOwnerFromPath("/repos/GipsyChef/agentdraft/git/refs/heads/main"),
    "GipsyChef",
  );
  assert.equal(
    extractOwnerFromPath("/orgs/GipsyChef/actions/runners"),
    "GipsyChef",
  );
  ...
  assert.equal(
    extractOwnerFromPath(
      "https://api.github.com/repos/GipsyChef/nightlamp/pulls/1",
    ),
    "GipsyChef",
  );
...
test("installationTokenIsValid requires a 90-second safety margin before expiry", () => {
  ...
  assert.equal(
    installationTokenIsValid({ token: "x", expiresAt: now + 3600 }, now),
    true,
  );
  // Just past the 90s safety margin: still valid (91s of headroom).
  assert.equal(
    installationTokenIsValid({ token: "x", expiresAt: now + 91 }, now),
    true,
  );
  // Right at the margin: not valid (refresh proactively).
  assert.equal(
    installationTokenIsValid({ token: "x", expiresAt: now + 90 }, now),
    false,
  );
  // Already expired.
  assert.equal(
    installationTokenIsValid({ token: "x", expiresAt: now - 1 }, now),
    false,
  );
});
...
test("recordRateLimit accumulates one bucket per (resource, installation) across scans", async () => {
  ...
  await scanMetrics.run(createScanMetrics(), async () => {
    recordRateLimit(fakeResponse({ limit: 5000, remaining: 4999, reset }), {
      installationKey: "cigan1",
    });
    recordRateLimit(fakeResponse({ limit: 6450, remaining: 5475, reset }), {
      installationKey: "gipsychef",
    });
  });
  await scanMetrics.run(createScanMetrics(), async () => {
    recordRateLimit(fakeResponse({ limit: 5000, remaining: 4998, reset }), {
      installationKey: "siftfy",
    });
  });
...
test("snapshotRateLimit picks tightest by ratio across installations", () => {
  ...
  recordRateLimit(
    fakeResponse({ limit: 5000, remaining: 4999, reset, used: 1 }),
    { installationKey: "cigan1" },
  );
  recordRateLimit(
    fakeResponse({ limit: 6450, remaining: 1000, reset, used: 5450 }),
    { installationKey: "gipsychef" },
  );
  recordRateLimit(
    fakeResponse({ limit: 5000, remaining: 4500, reset, used: 500 }),
    { installationKey: "siftfy" },
  );
...
test("snapshotRateLimit single-bucket case mirrors PAT mode", () => {
  ...
  const reset = Math.floor(Date.now() / 1000) + 3600;
  recordRateLimit(
    fakeResponse({ limit: 5000, remaining: 3068, reset, used: 1932 }),
    { installationKey: "pat" },
  );
  const snap = snapshotRateLimit(createScanMetrics());
...
test("snapshotRateLimit picks tightest across mixed resources and installations", () => {
  ...
  const reset = Math.floor(Date.now() / 1000) + 3600;
  recordRateLimit(
    fakeResponse({ limit: 6450, remaining: 5000, reset, used: 1450 }),
    { installationKey: "gipsychef" },
  );
...
test("recordRateLimit ratchets remaining downward within the same reset window", () => {
  resetObservedRateBuckets();
  const reset = Math.floor(Date.now() / 1000) + 3600;
  recordRateLimit(fakeResponse({ limit: 5000, remaining: 4500, reset }), {
    installationKey: "cigan1",
  });
  // A stale response with a higher `remaining` arrives — must not overwrite the lower value.
  recordRateLimit(fakeResponse({ limit: 5000, remaining: 4700, reset }), {
    installationKey: "cigan1",
  });
  const snap = snapshotRateLimit(createScanMetrics());
  ...
  recordRateLimit(
    fakeResponse({ limit: 5000, remaining: 5000, reset: newerReset }),
    { installationKey: "cigan1" },
  );
...
function setupHistoryEnv() {
  const tmpDir = mkdtempSync(join(tmpdir(), "github-monitor-history-"));
  process.env.HISTORY_ENABLED = "1";
  process.env.HISTORY_DIR = tmpDir;
  return {
    tmpDir,
    cleanup: () => {
...
test("history endpoints return enabled flag and entries", async () => {
  const { tmpDir, cleanup } = setupHistoryEnv();
  const previousFetch = globalThis.fetch;
  const previousToken = process.env.GITHUB_TOKEN;
  process.env.GITHUB_TOKEN = "test-token";
  ...
  try {
    ...
    const scansRes = await previousFetch(`${base}/api/history/scans`);
    const scansData = await scansRes.json();
    assert.equal(scansRes.status, 200);
    assert.equal(scansData.enabled, true);
    assert.equal(scansData.entries.length, 2);

    const summaryRes = await previousFetch(`${base}/api/history/summary`);
    const summaryData = await summaryRes.json();
    assert.equal(summaryRes.status, 200);
    assert.equal(summaryData.enabled, true);
    assert.equal(summaryData.totals.scans, 2);
    assert.ok(summaryData.buckets.length >= 1);
  } finally {
    await new Promise((resolve, reject) =>
      testServer.close((error) => (error ? reject(error) : resolve())),
    );
...
test("history endpoints return empty when disabled", async () => {
  const previousFetch = globalThis.fetch;
  const previousToken = process.env.GITHUB_TOKEN;
  process.env.GITHUB_TOKEN = "test-token";
  delete process.env.HISTORY_ENABLED;
  ...
  try {
    ...
    const scansData = await scansRes.json();
    assert.equal(scansRes.status, 200);
    assert.equal(scansData.enabled, false);
    assert.deepEqual(scansData.entries, []);

    const summaryRes = await previousFetch(`${base}/api/history/summary`);
    const summaryData = await summaryRes.json();
    assert.equal(summaryRes.status, 200);
    assert.equal(summaryData.enabled, false);
    assert.deepEqual(summaryData.buckets, []);
    assert.deepEqual(summaryData.totals, {});
  } finally {
    await new Promise((resolve, reject) =>
      testServer.close((error) => (error ? reject(error) : resolve())),
    );
